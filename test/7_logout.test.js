...
describe('POST /api/me/logout', () => {
  it('should be able to get 401 when wrong token', async () => {
    const meResult = await request(app).post('/api/me/logout').set('Authorization', `Bearer `)
    // Check status code
  ...
  it('should be able to logout my account', async () => {
    const auth = await request(app).post('/api/auth').send(credential.correct).expect(200)
    const myToken = auth.body.token;

    const meResult = await request(app).post('/api/me/logout').set('Authorization', `Bearer ${myToken}`)
    // Check status code
